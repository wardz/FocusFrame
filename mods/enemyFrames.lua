...
local showPortraitDebuff = function()
    local prioBuff = SPELLCASTINGCOREgetPrioBuff(CURR_FOCUS_TARGET, 1)[1]
...
local FocusExists = Focus.FocusExists
local refresh = 0
...
f:SetScript("OnUpdate", function()
    ...
    if refresh < 0 then
        if ENEMYFRAMESPLAYERDATA.targetPortraitDebuff and FocusExists() then
            showPortraitDebuff()
        else
